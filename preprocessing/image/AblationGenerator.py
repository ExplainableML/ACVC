...
class AblationGenerator:
    ...
    def get_severity(self):
        return 1 #np.random.randint(1, 6)
    ...
    def all(self, x):
        i = np.random.randint(0, 22) if self.corruption_mode == "all++" else np.random.randint(0, 19)
        corruption_func = {0: "fog",
                           1: "snow",
                           2: "frost",
                           3: "zoom_blur",
    ...
    def corruption(self, x, segmentation_mask=None):
        if self.rand_aug and np.random.uniform(0, 1) > 0.5:
            ...
            super_funcs = {"weather": self.weather,
                           "blur": self.blur,
                           "noise": self.noise,
                           ...
                           "all++": self.all}
            x_ = np.copy(x)
            corruption_mode = self.corruption_mode.split("_")[0]

            # Apply corruption
            if corruption_mode in super_funcs:
                x_ = super_funcs[corruption_mode](x_)
            else:
                x_ = self.apply_corruption(x, corruption_mode)
    ...
    def test(self, x, img_id):

        for f in get_corruption_names("all"):
            x_ = np.copy(x)
            x_ = self.apply_corruption(x_, f)
            x_.save("example_%d_%s.jpeg" % (img_id, f))
            print("%s testing is done." % f)

        for f in ["high_pass_filter", "constant_amplitude", "phase_scaling"]:
            x_ = np.copy(x)
            x_ = self.apply_corruption(x_, f)
            x_.save("example_%d_%s.jpeg" % (img_id, f))
            print("%s testing is done." % f)
